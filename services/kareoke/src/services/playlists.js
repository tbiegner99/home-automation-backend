...
import PlaylistDatasource from '../datasource/playlists/datasource.js';
...
const PLAYLIST_FILE = path.join(
    process.env.PLAYLIST_FILE_LOCATION || '',
    'upNext.txt'
);
...
class PlaylistService {
    ...
    async updatePlaylistFile(playlistId) {
        const playlistItems =
            await this.playlistDatasource.getTopNItemsOfPlaylist(
                playlistId,
        ...
        await fs.writeFile(PLAYLIST_FILE, fileString, { flag: 'w' });
    }
    ...
    async enqueuePlaylistItem(playlistId, playlistItem) {
        const lastPosition =
            await this.playlistDatasource.getLastPositionFroPlaylist(
                playlistId
            );
    ...
    async addItemAtFront(playlistId, playlistItem) {
        const firstPosition =
            await this.playlistDatasource.getFirstPositionForPlaylist(
                playlistId
            );
    ...
    async addItemAfter(playlistId, position, playlistItem) {
        const upperBoundPosition =
            await this.playlistDatasource.getNextPositionAfter(
                playlistId,
                position
            );
    ...
    async moveUp(playlistId, itemPositionToMove) {
        const twoItemsBefore =
            await this.playlistDatasource.getItemPositionForMoveUpOperation(
                playlistId,
                itemPositionToMove
            );
        if (!twoItemsBefore) {
