...
const getClosestHalfHour = () => {
    ...
    if (now.minutes >= 30) {
        return now.startOf('hour').add(30, 'minutes').utc().toDate();
    }
    return now.startOf('hour').utc().toDate();
};

class TVGuideDatasource {
    constructor() {
        this.client = DBClientFactory.getClient(Databases.Schemas.TV);
    }
...
export default TVGuideDatasource;
